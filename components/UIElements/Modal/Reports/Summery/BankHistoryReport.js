...
export default function BankHistoryReport({ docName, reportName } = {}) {
  ...
  const { data: bankHistoryReport } = GetReportSettingValueByName(reportName);
  ...
  const buildCrystalReportUrl = () => {
    const params = new URLSearchParams({
      ...
      reportName: bankHistoryReport || "",
      fromDate: fromDate || "",
      toDate: toDate || "",
      warehouseId: warehouseId || "",
      currentUser: name || "",
      bankId: String(bankId || 0),
  ...
  const handleSubmit = () => {
    if (!fromDate || !toDate || !bankId) return;
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={12}>
              <ReportFilterSelect
                filterType="bank"
                value={bankId}
                selectedLabel={bankId ? bankName : ""}
                onChange={(id, label) => {
                  setBankId(id ?? 0);
                  setBankName(id ? label || "" : "");
                }}
                allowAll={false}
                label="Select Bank"
                required
