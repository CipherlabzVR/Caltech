...
export default function StockMovement({ docName, reportName } = {}) {
  ...
  const { data: StockMovementReport } = GetReportSettingValueByName(reportName);
  ...
  const buildCrystalReportUrl = () => {
    const params = new URLSearchParams({
      ...
      reportName: StockMovementReport || "",
      fromDate: fromDate || "",
      toDate: toDate || "",
  ...
  return (
    <>
      ...
      <Modal
        open={open}
        onClose={handleClose}
        aria-labelledby="stock-movement-report-title"
        aria-describedby="stock-movement-report-description"
      >
