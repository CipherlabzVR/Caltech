...
const STATUS_OPTIONS = [
  { value: 0, label: ALL_LABELS.status },
...
export default function PurchaseOrderNotesSummaryReport({ docName, reportName } = {}) {
  ...
  const statusName =
    STATUS_OPTIONS.find((option) => option.value === Number(status))?.label || ALL_LABELS.status;

  const buildCrystalReportUrl = () => {
    const params = new URLSearchParams({
      InitialCatalog: Catelogue,
      reportName: purchaseOrderNotesSummaryReport || "",
      fromDate: fromDate || "",
      toDate: toDate || "",
  ...
  const openHtmlReport = () => {
    ...
    openSummaryHtmlReport("/reports/purchase-order-notes-summary/print", params, reportMode === REPORT_MODE.EXCEL);
  };
  ...
  const modalTitle = summaryModalTitle("Purchase Order Notes Summary Report", reportMode);
  ...
  return (
    <>
      ...
      <Modal
        open={open}
        onClose={handleClose}
        aria-labelledby="po-notes-summary-report-title"
        aria-describedby="po-notes-summary-report-description"
      >
