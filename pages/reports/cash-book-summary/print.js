...
export default function CashBookSummaryPrintPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(`${BASE_URL}/CustomerPaymentHistory/GetCashBookSummary?${params}`, {
          method: "GET",
          headers: authHeaders(),
        });
        ...
        if (!res.ok) {
          toast.error(json?.message || "Failed to load cash book summary.");
          setRows([]);
        } else {
        ...
        console.error("[CashBookSummaryPrint] load failed", e);
    ...
    load();
  }, [router.isReady, fromDate, toDate, customerId]);
  ...
  const tokenMap = useMemo(
    () => ({
      ...
      customerFilter: toFilterLabel(router.query.customerName, "—"),
  ...
  return (
    <>
      <TemplatePrintFrame
        ...
        loadingText="Loading cash book summary…"
