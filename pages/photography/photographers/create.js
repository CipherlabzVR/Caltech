...
export default function AddPhotographer({ fetchItems }) {
  ...
  const handleSubmit = (values) => {
    ...
    fetch(`${BASE_URL}/Photographer/CreatePhotographer`, {
      method: "POST",
      ...
      .then((data) => {
        ...
        if (sc === 200 || sc === "SUCCESS") {
          toast.success(msg || "Photographer created");
          handleClose();
          fetchItems?.();
          if (result?.temporaryPassword || result?.TemporaryPassword) {
            setCreds({
              email: result.email ?? result.Email ?? values.Email,
              temporaryPassword: result.temporaryPassword ?? result.TemporaryPassword,
              emailSent: result.emailSent ?? result.EmailSent ?? false,
            });
          }
        } else {
          toast.error(msg || "Failed to create photographer");
        }
      })
      .catch((error) => {
                ...
                <Grid spacing={1} container>
                  <Grid item xs={12}>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                      A login account will be created using the email. A temporary password will be shown after save.
                    </Typography>
                  </Grid>
