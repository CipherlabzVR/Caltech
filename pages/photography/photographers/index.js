...
import AddPhotographer from "./create";
import EditPhotographer from "./edit";
import { formatDate } from "@/components/utils/formatHelper";
import IsPermissionEnabled from "@/components/utils/IsPermissionEnabled";
import AccessDenied from "@/components/UIElements/Permission/AccessDenied";

const CATEGORY_ID = 227;
...
export default function PhotographerList() {
  ...
  const controller = "Photographer/DeletePhotographer";
  ...
  const {
    ...
    setSearch,
    fetchData: fetchList,
  } = usePaginatedFetch("Photographer/GetAllPhotographerPaged", "", 10, false, false);
    ...
    <>
      ...
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 1, md: 1, lg: 1, xl: 2 }}>
        ...
        <Grid item xs={12} lg={8} mb={1} display="flex" justifyContent="end" order={{ xs: 1, lg: 2 }}>
          {create ? <AddPhotographer fetchItems={() => fetchList()} /> : ""}
        </Grid>
        <Grid item xs={12} order={{ xs: 3, lg: 3 }}>
          <TableContainer component={Paper}>
            <Table aria-label="photographers" className="dark-table">
              <TableHead>
                <TableRow>
                  <TableCell>#</TableCell>
                ...
                {!list || list.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10}>
                      <Typography color="error">No photographers available</Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  list.map((item, index) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>{item.defaultTeam ? item.defaultTeam.name : "-"}</TableCell>
                      <TableCell>
                        {item.isVideography ? (
                      ...
                      <TableCell align="right">
                        {update ? <EditPhotographer item={item} fetchItems={fetchList} /> : ""}
                        {remove ? (
