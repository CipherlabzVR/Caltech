...
import AddTeam from "./create";
import EditTeam from "./edit";
import { formatDate } from "@/components/utils/formatHelper";
import IsPermissionEnabled from "@/components/utils/IsPermissionEnabled";
import AccessDenied from "@/components/UIElements/Permission/AccessDenied";

const CATEGORY_ID = 226;

export default function PhotographyTeamList() {
  ...
  const controller = "PhotographyTeam/DeleteTeam";
  ...
  const {
    ...
    setSearch,
    fetchData: fetchList,
  } = usePaginatedFetch("PhotographyTeam/GetAllTeamPaged", "", 10, false, false);
    ...
    <>
      ...
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 1, md: 1, lg: 1, xl: 2 }}>
        ...
        <Grid item xs={12} lg={8} mb={1} display="flex" justifyContent="end" order={{ xs: 1, lg: 2 }}>
          {create ? <AddTeam fetchItems={() => fetchList()} /> : ""}
        </Grid>
        <Grid item xs={12} order={{ xs: 3, lg: 3 }}>
          <TableContainer component={Paper}>
            <Table aria-label="teams" className="dark-table">
              <TableHead>
                <TableRow>
                  <TableCell>#</TableCell>
              ...
              <TableBody>
                {!list || list.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7}>
                      <Typography color="error">No teams available</Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  list.map((item, index) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell align="right">
                        {update ? <EditTeam item={item} fetchItems={fetchList} /> : ""}
                        {remove ? (
