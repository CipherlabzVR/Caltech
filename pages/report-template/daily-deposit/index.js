...
const TEMPLATE_NAME = "Daily Deposit Print Template";
...
const SAMPLE_DATA = {
  ...
  documentNo: "DD-000045",
  depositDate: "30-Jun-2026",
  warehouseName: "Main Warehouse",
  createdBy: "Nimal Silva",
  totalEntries: "3",
  totalAmount: "152,500.00",
};

const SAMPLE_ROWS = [
  { supplier: "Supplier A", bank: "Commercial Bank", bankAccountNumber: "8001234567", amount: "85,000.00" },
  { supplier: "Supplier B", bank: "Sampath Bank", bankAccountNumber: "1102233445", amount: "42,500.00" },
  { supplier: "Profit Account", bank: "BOC", bankAccountNumber: "7700998877", amount: "25,000.00" },
];
  ...
  const fetchTemplate = useCallback(async () => {
    try {
      ...
      if (response.ok && data) {
        ...
        toast.error(data?.message || "Failed to load the Daily Deposit print template.");
      }
    } catch (error) {
      console.error("Error loading report template:", error);
      toast.error("Failed to load the Daily Deposit print template.");
    } finally {
      setLoading(false);
    }
  }, [authHeaders]);
  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        <h1>Daily Deposit Print Template</h1>
        <ul>
          <li>
          ...
          <li>Daily Deposit Print Template</li>
        </ul>
      </div>
            ...
            <Paper sx={{ p: 1.5, height: "100%" }}>
              ...
              >
                <Box
                  component="iframe"
                  ref={iframeRef}
                  title="Daily Deposit template preview"
      ...
      <Dialog open={resetOpen} onClose={() => (saving ? null : setResetOpen(false))}>
        ...
        <DialogContent>
          <DialogContentText>
            This will permanently delete the custom Daily Deposit print template and restore the
