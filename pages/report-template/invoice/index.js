...
const TEMPLATE_NAME = "Invoice Print Template";
...
const SAMPLE_ROWS = [
  { productName: "A4 Copy Paper 80gsm", productCode: "ITM-1001", qty: "20", unitPrice: "850.00", discountPercentage: "0.00", discountAmount: "0.00", lineTotal: "17,000.00" },
  { productName: "Blue Ball Pen", productCode: "ITM-1002", qty: "100", unitPrice: "45.00", discountPercentage: "5.00", discountAmount: "225.00", lineTotal: "4,275.00" },
  { productName: "Stapler Heavy Duty", productCode: "ITM-1003", qty: "10", unitPrice: "1,250.00", discountPercentage: "0.00", discountAmount: "0.00", lineTotal: "12,500.00" },
];
...
const buildSampleRows = () =>
  SAMPLE_ROWS.map(
    (r) => `<tr>
      ...
      <td class="num">${r.discountPercentage}</td>
  ...
  const fetchTemplate = useCallback(async () => {
    try {
      ...
      if (response.ok && data) {
        ...
        toast.error(data?.message || "Failed to load the Invoice print template.");
      }
    } catch (error) {
      console.error("Error loading report template:", error);
      toast.error("Failed to load the Invoice print template.");
    } finally {
      setLoading(false);
    }
  }, [authHeaders]);
  ...
  return (
    <>
      ...
      <Dialog open={resetOpen} onClose={() => (saving ? null : setResetOpen(false))}>
        ...
        <DialogContent>
          <DialogContentText>
            This will permanently delete the custom Invoice print template and restore the
