...
const REPORT_KEY = "PO";
const TEMPLATE_NAME = "Purchase Order Print Template";
...
const SAMPLE_ROWS = [
  { productName: "A4 Copy Paper 80gsm", batch: "B-2207", expDate: "-", orderQty: "50", costPrice: "850.00", discountRate: "0.00", discountAmount: "0.00", lineTotal: "42,500.00" },
  { productName: "Blue Ball Pen", batch: "B-3310", expDate: "-", orderQty: "40", costPrice: "45.00", discountRate: "5.00", discountAmount: "90.00", lineTotal: "1,710.00" },
  { productName: "Stapler Heavy Duty", batch: "B-9921", expDate: "-", orderQty: "30", costPrice: "1,250.00", discountRate: "0.00", discountAmount: "0.00", lineTotal: "37,500.00" },
];
  ...
  const fetchTemplate = useCallback(async () => {
    try {
      ...
      if (response.ok && data) {
        ...
        toast.error(data?.message || "Failed to load the Purchase Order print template.");
      }
    } catch (error) {
      console.error("Error loading report template:", error);
      toast.error("Failed to load the Purchase Order print template.");
    } finally {
      setLoading(false);
    }
  }, [authHeaders]);
  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        <h1>Purchase Order Print Template</h1>
        <ul>
          <li>
          ...
          <li>Purchase Order Print Template</li>
        </ul>
      </div>
            ...
            <Paper sx={{ p: 1.5, height: "100%" }}>
              ...
              >
                <Box
                  component="iframe"
                  ref={iframeRef}
                  title="Purchase Order template preview"
      ...
      <Dialog open={resetOpen} onClose={() => (saving ? null : setResetOpen(false))}>
        ...
        <DialogContent>
          <DialogContentText>
            This will permanently delete the custom Purchase Order print template and restore the
