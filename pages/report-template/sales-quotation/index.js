...
const TEMPLATE_NAME = "Sales Quotation Print Template";
...
const SAMPLE_ROWS = [
  { productName: "A4 Copy Paper 80gsm", productCode: "ITM-1001", qty: "20", sellingPrice: "900.00", lineTotal: "18,000.00" },
  { productName: "Blue Ball Pen", productCode: "ITM-1002", qty: "5", sellingPrice: "60.00", lineTotal: "300.00" },
  { productName: "Stapler Heavy Duty", productCode: "ITM-1003", qty: "5", sellingPrice: "1,400.00", lineTotal: "7,000.00" },
];
  ...
  const fetchTemplate = useCallback(async () => {
    try {
      ...
      if (response.ok && data) {
        ...
        toast.error(data?.message || "Failed to load the Sales Quotation print template.");
      }
    } catch (error) {
      console.error("Error loading report template:", error);
      toast.error("Failed to load the Sales Quotation print template.");
    } finally {
      setLoading(false);
    }
  }, [authHeaders]);
  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        <h1>Sales Quotation Print Template</h1>
        <ul>
          <li>
          ...
          <li>Sales Quotation Print Template</li>
        </ul>
      </div>
            ...
            <Paper sx={{ p: 1.5, height: "100%" }}>
              ...
              >
                <Box
                  component="iframe"
                  ref={iframeRef}
                  title="Sales Quotation template preview"
      ...
      <Dialog open={resetOpen} onClose={() => (saving ? null : setResetOpen(false))}>
        ...
        <DialogContent>
          <DialogContentText>
            This will permanently delete the custom Sales Quotation print template and restore the
