...
const buildDepositLineTokenMap = (item) => ({
  supplier: item.supplier || "-",
  bank: item.bankName || item.bank || item.bankId || "-",
...
const buildLineItemsRows = (items) => {
  ...
  return items
    .map((item) => {
      const t = buildDepositLineTokenMap(item);
      return `<tr>
        <td>${escapeHtml(t.supplier)}</td>
        <td>${escapeHtml(t.bank)}</td>
        <td>${escapeHtml(t.bankAccountNumber)}</td>
...
export default function DailyDepositPrintPage() {
  ...
  const finalHtml = useMemo(() => {
    ...
    const lineTokenMaps = lineItems.map(buildDepositLineTokenMap);
    ...
    >
      ...
      >
        ...
        >
          ...
          <PrintExcelButton
            iframeRef={iframeRef}
            downloadName={`DailyDeposit_${depositData?.documentNo || documentNumber || "document"}`}
            disabled={isLoading || !finalHtml}
          />
          {/* <Button
            variant="outlined"
