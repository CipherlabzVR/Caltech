...
export default function SalesQuotation() {
  ...
  const { navigate, create, update, remove, print, customPrint, whatsAppShare, permissionsLoading } =
  ...
  const {
    ...
    handleChangePage,
    handleChangeRowsPerPage } = usePaginatedFetch("SalesQuotation/GetAll", "", 10, false);
  const restoredFromQueryRef = useRef(false);

  useEffect(() => {
    if (!router.isReady || restoredFromQueryRef.current) return;
    ...
    if (queryPage === DEFAULT_PAGE && querySize === DEFAULT_PAGE_SIZE) {
      if (isReload) {
    ...
    fetchQuotationList(queryPage, search, querySize);
  }, [router.isReady]);
