...
const buildSalesQuotationLineTokenMap = (item) => ({
...
export default function SalesQuotationPrintPage() {
  ...
  const finalHtml = useMemo(() => {
    ...
    const lineTokenMaps = lineItems.map(buildSalesQuotationLineTokenMap);
          ...
          <Box sx={{ display: "flex", flexDirection: "column", alignItems: { xs: "stretch", sm: "flex-end" }, gap: 1 }}>
            <Box display="flex" gap={1} flexWrap="wrap" justifyContent="flex-end">
              ...
              <PrintExcelButton
                ...
                downloadName={`Sales_Quotation_${salesQuotationData?.documentNo || documentNumber || "document"}`}
                disabled={loadingSalesQuotation || !salesQuotationData}
              />
