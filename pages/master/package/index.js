  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        ...
        <ul>
          <li>
            <Link href="/master/package/">Packages</Link>
          </li>
        </ul>
      </div>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 1, md: 1, lg: 1, xl: 2 }}>
        ...
        <Grid item xs={12} lg={8} mb={1} display="flex" justifyContent="end" order={{ xs: 1, lg: 2 }}>
          {create ? <AddPackage fetchItems={fetchPackageList} /> : ""}
        </Grid>
                ...
                {packageList.length === 0 ? (
                  ...
                  packageList.map((item, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>{formatCurrency(item.packageCost)}</TableCell>
                      <TableCell>
                        {item.imageURL != "" ? <a href={item.imageURL} target="_blank">View</a> : ""}
                      </TableCell>
                      <TableCell>
                        {item.isActive ? (
                      ...
                      <TableCell align="right">
                        {update ? <EditPackage fetchItems={fetchPackageList} item={item} /> : ""}
                        {remove ? <DeleteConfirmationById id={item.id} controller={controller} fetchItems={fetchPackageList} /> : ""}
                      </TableCell>
                    </TableRow>
                  ))
                )}
