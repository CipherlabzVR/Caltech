...
const ShipmentCreate = () => {
    ...
    const invalidQtyLine = selectedRows.find((row) => {
      const poQty = Number(row.quantity ?? row.qty) || 0;
      const orderedQty = Number(row.orderedQty) || 0;
      const remaining = poQty - orderedQty;
      const requestQty = Number(row.requestQty) || 0;
      return requestQty <= 0 || requestQty > remaining;
    });

    if (invalidQtyLine) {
      const poQty = Number(invalidQtyLine.quantity ?? invalidQtyLine.qty) || 0;
      const orderedQty = Number(invalidQtyLine.orderedQty) || 0;
      const remaining = poQty - orderedQty;
      toast.error(
        `Invalid quantity for ${invalidQtyLine.name}. Quantity must be greater than 0 and not exceed remaining (${remaining}).`
      );
      return;
    }
