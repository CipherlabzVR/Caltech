...
export default function Dispatch() {
    ...
    const fetchDispachList = async (page = 1, search = "", size = pageSize) => {
        try {
            const token = localStorage.getItem("token");
            const skip = (page - 1) * size;
            const query = `${BASE_URL}/StockBalance/GetAllStockDispatchGrouped?SkipCount=${skip}&MaxResultCount=${size}&Search=${search || "null"}`;
            ...
            setDispatchList(data.result.items || []);
    ...
    const openStockDispatchPrintPopup = (group) => {
        ...
        window.open(
            ...
            `stock-dispatch-print-${group?.firstLineId ?? firstLine?.id ?? "group"}`,
    ...
    return (
        <>
                                                ...
                                                <Box sx={{ display: "flex", gap: 0.5 }}>
                                                    ...
                                                    {print ? (
                                                        <Tooltip title="Print">
                                                            <IconButton
                                                                size="small"
                                                                onClick={() => openStockDispatchPrintPopup(group)}
                                                            >
            ...
            <Dialog
                ...
                fullWidth
            >
                <DialogTitle>Stock Dispatch Details</DialogTitle>
                                    ...
                                    <TableBody>
                                        {(viewGroup.lines || []).map((line, index) => (
                                            <TableRow key={line.id || index}>
                                                ...
                                                <TableCell align="right">{formatCurrency(line.costPrice)}</TableCell>
                                                <TableCell align="right">{formatCurrency(line.sellingPrice)}</TableCell>
                                                <TableCell align="right">{line.dispatchQuantity}</TableCell>
