...
export default function CreatePostingRule() {
  ...
  const [lines, setLines] = useState([
    { chartOfAccountId: "", transactionType: 2, description: "", sortOrder: 1 },
    { chartOfAccountId: "", transactionType: 1, description: "", sortOrder: 2 },
  ]);
  ...
  const removeLine = (index) => {
    if (lines.length <= 2) {
      toast.error("A posting rule must have at least 2 lines.");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/PostingRule/Create`, {
        method: "POST",
      ...
      if (data.statusCode === 200) {
        toast.success("Posting Rule created successfully.");
        setTimeout(() => router.push("/finance/posting-rules"), 1000);
      } else {
        toast.error(data.message || "Failed to create posting rule.");
      }
    } catch (error) {
          ...
          <Paper sx={{ p: 3 }}>
            <Grid container spacing={2}>
              <Grid item xs={12} md={4}>
                ...
                <TextField fullWidth size="small" placeholder="e.g. Sales Invoice Posting" value={ruleName} onChange={(e) => setRuleName(e.target.value)} />
              </Grid>
              ...
              <Grid item xs={12} md={5}>
                ...
                <TextField fullWidth size="small" placeholder="Optional description" value={description} onChange={(e) => setDescription(e.target.value)} />
              </Grid>
            </Grid>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 2 }}>
            ...
            <Box display="flex" justifyContent="space-between" mt={3}>
              ...
              <LoadingButton variant="contained" size="small" loading={loading} onClick={handleSubmit}>Save Rule</LoadingButton>
            </Box>
          </Paper>
        </Grid>
      </Grid>
