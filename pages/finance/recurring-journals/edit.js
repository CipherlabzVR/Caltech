...
export default function EditRecurringJournal() {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        ...
        const response = await fetch(`${BASE_URL}/RecurringJournal/GetAll?SkipCount=0&MaxResultCount=200&Search=null`, {
          headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        });
        const data = await response.json();
        const item = data.result.items.find((r) => r.id === parseInt(id));
        if (item) {
          setName(item.name);
          ...
          setLines(item.lines.map((l) => ({
            chartOfAccountId: l.chartOfAccountId, debitAmount: l.debitAmount, creditAmount: l.creditAmount, description: l.description || "",
          })));
          setLoaded(true);
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${BASE_URL}/RecurringJournal/Update/${id}`, {
        method: "PUT",
        body: JSON.stringify({
      ...
      const data = await response.json();
      if (data.statusCode === 200) { toast.success("Updated successfully."); setTimeout(() => router.push("/finance/recurring-journals"), 1000); }
      else toast.error(data.message || "Failed.");
    } catch { toast.error("An error occurred."); } finally { setLoading(false); }
  };

  if (!loaded) return <div className={styles.pageTitle}><h1>Loading...</h1></div>;
  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        ...
        <ul><li><Link href="/finance/recurring-journals/">Recurring Journals</Link></li><li>Edit</li></ul>
      </div>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 2 }}>
                ...
                <TableBody>
                  {lines.map((line, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell><TextField fullWidth size="small" value={line.description} onChange={(e) => updateLine(index, "description", e.target.value)} /></TableCell>
            ...
            <Box display="flex" justifyContent="space-between" mt={3}>
              ...
              <LoadingButton variant="contained" size="small" loading={loading} disabled={!isBalanced} onClick={handleSubmit}>Update</LoadingButton>
            </Box>
          </Paper>
        </Grid>
      </Grid>
