...
export default function EditDoubleEntry() {
    ...
    const fetchEntry = async () => {
      try {
        ...
        if (entry.status !== 1) {
          ...
          setTimeout(() => router.push(`/finance/double-entry/${id}`), 1000);
          return;
        }
  ...
  const removeLine = (index) => {
    if (lines.length <= 2) { toast.error("A double entry must have at least 2 lines."); return; }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/DoubleEntry/Update/${id}`, {
        method: "PUT",
      ...
      if (data.statusCode === 200) {
        toast.success("Double Entry updated successfully.");
        setTimeout(() => router.push(`/finance/double-entry/${id}`), 1000);
      } else {
  ...
  return (
    <>
      ...
      <div className={styles.pageTitle}>
        <h1>Edit Double Entry - {documentNo}</h1>
        <ul>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 2 }}>
            ...
            <Box display="flex" justifyContent="space-between" mt={3}>
              <Button variant="contained" color="error" size="small" onClick={() => router.push(`/finance/double-entry/${id}`)}>Cancel</Button>
