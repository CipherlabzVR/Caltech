...
import { Typography, Button, Chip, Box, TextField, Dialog, DialogTitle, DialogContent, DialogActions } from "@mui/material";
...
export default function DoubleEntryDetail() {
  ...
  const fetchEntry = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/DoubleEntry/GetById/${id}`, {
        headers: {
  ...
  const handlePost = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${BASE_URL}/DoubleEntry/Post/${id}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
      ...
      if (data.statusCode === 200) {
        toast.success("Double Entry posted successfully.");
        fetchEntry();
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${BASE_URL}/DoubleEntry/Delete/${id}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
      ...
      if (data.statusCode === 200 || data.result?.statusCode === 200) {
        toast.success("Double Entry deleted.");
  ...
  const handleReverse = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${BASE_URL}/DoubleEntry/Reverse`, {
        method: "POST",
        body: JSON.stringify({
          doubleEntryId: parseInt(id),
      ...
      const data = await response.json();
      if (data.statusCode === 200) {
        toast.success("Double Entry reversed successfully.");
  ...
  return (
    <>
      <ToastContainer />
      <div className={styles.pageTitle}>
        <h1>Double Entry - {entry.documentNo}</h1>
        <ul>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="h6" mb={2}>Entry Lines</Typography>
            <TableContainer>
            ...
            <Box display="flex" justifyContent="space-between" mt={3}>
              <Button variant="outlined" size="small" onClick={() => router.push("/finance/double-entry")}>
                Back to List
              </Button>
              <Box display="flex" gap={1}>
                ...
                <LoadingButton variant="outlined" size="small" startIcon={<ContentCopyIcon />} loading={duplicateLoading} onClick={async () => {
                  ...
                  try {
                    const token = localStorage.getItem("token");
                    const resp = await fetch(`${BASE_URL}/DoubleEntry/Duplicate/${id}`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" } });
                    const data = await resp.json();
                    if (data.statusCode === 200) {
                      toast.success(`Duplicated as ${data.result.documentNo}`);
                      setTimeout(() => router.push(`/finance/double-entry/${data.result.id}`), 1000);
                ...
                {entry.status === 1 && (
                  <>
                    <Button variant="outlined" size="small" color="primary" startIcon={<EditIcon />} onClick={() => router.push(`/finance/double-entry/edit?id=${id}`)}>
                      Edit
                    </Button>
      ...
      <Dialog open={reverseOpen} onClose={() => setReverseOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Reverse Double Entry</DialogTitle>
        <DialogContent>
