...
export default function JournalEntries() {
  ...
  const fetchEntries = async (pg = 1, search = "", size = pageSize, status = statusFilter, periodId = fiscalPeriodId) => {
    try {
      ...
      let query = `${BASE_URL}/JournalEntry/GetAll?SkipCount=${skip}&MaxResultCount=${size}&Search=${search || "null"}`;
    ...
    <>
      ...
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 1, md: 1, lg: 1, xl: 2 }}>
        <Grid item xs={12} mb={1} display="flex" justifyContent="end" order={{ xs: 1, lg: 1 }}>
          {create ? (
            <Button variant="outlined" onClick={() => router.push("/finance/journal-entry/create")}>
              + New Journal Entry
            </Button>
          ) : null}
        </Grid>
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell align="center">
                        <Tooltip title="View Details" placement="top">
                          <IconButton size="small" onClick={() => router.push(`/finance/journal-entry/${item.id}`)}>
