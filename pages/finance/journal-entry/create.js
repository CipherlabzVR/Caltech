...
const isDebitNormal = (accountType) => {
  switch (accountType) {
    ...
    case 13: // Tax
    case 14: // Retained Earnings
      return false; // Credit-normal
    default:
      return true;  // Debit-normal (Payment/Expense, FixedAssets, Bank, AR, COGS, OtherExpense)
  }
};
...
export default function CreateJournalEntry() {
  ...
  const canFillRemaining = (line) => {
    ...
    return lineDebit === 0 && lineCredit === 0 && totalDebit + totalCredit > 0 && totalDebit !== totalCredit;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/JournalEntry/Create`, {
        method: "POST",
      ...
      if (data.statusCode === 200) {
        ...
        toast.error(data.message || "Failed to create journal entry.");
      }
    } catch (error) {
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 2 }}>
                        ...
                        {selectedAcc ? (
                          <Box display="flex" flexDirection="column" alignItems="center" gap={0.3}>
                            <Typography variant="caption" color="text.secondary" lineHeight={1.2}>
                              {ChartOfAccountType(selectedAcc.accountType)}
                            </Typography>
                      ...
                      <TableCell align="center">
                        <Box display="flex" alignItems="center" gap={0.5}>
                          {canFillRemaining(line) && (
                            <Chip
                              icon={<BalanceIcon sx={{ fontSize: "14px !important" }} />}
                              label={`${formatCurrency(Math.abs(totalDebit - totalCredit))} ${totalDebit > totalCredit ? "Cr" : "Dr"}`}
                              size="small"
                              color="primary"
                              variant="outlined"
            ...
            <Box display="flex" justifyContent="space-between" mt={3}>
              <Button variant="contained" color="error" size="small" onClick={() => router.push("/finance/journal-entry")}>
                Cancel
              </Button>
              <LoadingButton
                variant="contained"
